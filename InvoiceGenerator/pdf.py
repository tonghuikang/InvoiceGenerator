...
class SimpleInvoice(BaseInvoice):
    ...
    def gen(self, filename, generate_qr_code=False):
        ...
        self._drawClient(self.TOP - 47, self.LEFT + 3)
    ...
    def _drawTitle(self):  # not drawing title
        # Up line
    ...
    def _drawAddress(self, top, left, width, height, header_string, address):
        ...
        story = [
            Paragraph(address.summary, header),
    ...
    def _drawItemsHeader(self,  TOP,  LEFT):
        ...
        if items_are_with_tax:
            i = 9
            self.pdf.drawString((LEFT + 73) * mm, (TOP - i) * mm, _(u'Qty'))
            self.pdf.drawString(
                (LEFT + 88) * mm,
                (TOP - i) * mm,
                _(u'Unit Price'),
...
''' # commented out since we are not concerned with taxes
